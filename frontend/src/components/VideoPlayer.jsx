    ...
    <div className="flex flex-col gap-6 max-md:flex-col">
      <div className="relative w-full bg-black/20">
        {JSON.parse(Cookies.get("auth"))["user_type"] === "lecturer" ? (
          ...
          <>
            <button
              className={`m-2 btn ${meetingMode === "virt-class" ? "btn-active text-white" : ""}`}
              onClick={() => {
                setMeetingMode("virt-class");
                document.getElementById("virtClassModal").showModal();
              }}
            ...
            <button
              className={`m-2 btn ${meetingMode === "peer-peer" ? "btn-active text-white" : ""}`}
              onClick={() => {
        ...
        <div className="modal-box">
          ...
          <div className="modal-action">
            <form method="dialog">
              ...
              >
                {JSON.parse(Cookies.get("auth"))["user_type"] === "lecturer"
                    ? "Start Classroom" : "Join Classroom"}
              </button>
            </form>
          </div>
        </div>
